...
function renderArtworks(filter = {}) {
    ...
    let filtered = artworks.filter(art => {
        // Filtrar por técnica
        if (filter.technique && filter.technique !== "" && art.technique !== filter.technique) return false;
        // Filtrar por búsqueda de título (case-insensitive)
        if (filter.search && filter.search.trim() !== "" && !art.title.toLowerCase().includes(filter.search.toLowerCase())) return false;
        return true;
    });
